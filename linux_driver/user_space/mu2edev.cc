...
{   TRACE( 0, "mu2edev::init called" );
	...
	{   m_ioc_get_info_t get_info;
	    ...
	    TRACE( 1, "%u:%u - num=%u size=%u hwIdx=%u, swIdx=%u"
		  , chn,dir
...
int mu2edev::read_data( int chn, void **buffer, int tmo_ms )
{
	...
	if (has_recv_data == 0)
	{   retsts = ioctl( devfd_, M_IOC_GET_INFO
			   , &mu2e_channel_info_[chn][C2S] );
	    if (retsts != 0) { perror( "M_IOC_GET_INFO" ); exit (1); }
	    has_recv_data = delta_( chn, C2S );
	    TRACE( 1, "mu2edev::read_data chn%d has_recv_data=%u", chn, has_recv_data );
	}
	else
	    TRACE( 1, "mu2edev::read_data chn%d:%d hwIdx=%u, swIdx=%u %u has_recv_data=%u (cached)"
		  , chn, C2S
		  , mu2e_channel_info_[chn][C2S].hwIdx, mu2e_channel_info_[chn][C2S].swIdx
		  , mu2e_channel_info_[chn][C2S].num_buffs
		  , has_recv_data );
    }
    retsts = has_recv_data; // TEMPORARILY
    return (retsts);
}
