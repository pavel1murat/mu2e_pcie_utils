...
void help()
{
	...
	std::cout << "\nFor more information on this compiler, look at the CFO's Compiler and Interpreter Mu2e Guides." << std::endl;
}
...
void newIOFiles()
{
	...
	std::cout << "DEFAULT Clock Speeed: " << clockSpeed << "Hz (" << atoi(clockSpeed.c_str()) / 1000000 << "MHz)" << std::endl;
...
void setIOFiles(int argc, char** argv) { //Sets the Input Output Files.
	...
	{
		throw std::runtime_error("Setup File (setup.in) didn't open. \nDoes it exist? \nIf not: run \"./CFO_Compiler --new\"");
	}
	if (!(inFile.is_open()))
	{
