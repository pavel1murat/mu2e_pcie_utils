...
void printHelpMsg()
{
	...
	std::cout << "Options are:" << std::endl
		...
		<< "    -C: CFO program needs to be compiled (i.e. not bitfile). If specified with -f, will not write compiled program to CFO" << std::endl
		<< "    -c: Clock speed for CFO (default 40000000)" << std::endl
		<< "    --cfo: Use cfo <num> (Defaults to DTCLIB_DTC if set, 0 otherwise, see ls /dev/mu2e* for available CFOs)" << std::endl
		;
	exit(0);
}
