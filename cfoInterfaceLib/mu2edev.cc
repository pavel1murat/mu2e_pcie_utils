...
int mu2edev::init(CFOLib::CFO_SimMode simMode)
{
    if (simMode != CFOLib::CFO_SimMode_Disabled) {
        ...
        for (unsigned dir = 0; dir < 2; ++dir)
        {
            ...
            get_info.chn = 0; get_info.dir = dir; get_info.tmo_ms = 0;
            ...
            TRACE(4, "mu2edev::init %u:%u - num=%u size=%u hwIdx=%u, swIdx=%u delta=%u"
                ...
                , mu2e_chn_info_delta_(0, dir, &mu2e_channel_info_) );
            ...
            off64_t offset = chnDirMap2offset(0, dir, map);
            ...
            // Enable DMA Engines
                {   uint16_t addr = DTC_Register_Engine_Control(0, dir);
                TRACE(17, "mu2edev::init write Engine_Control reg 0x%x", addr);
                write_register(DTC_Register_Engine_Control(0, dir), 0, 0x100);//bit 8 enable=1
                }
        }
#endif
    }
...
int mu2edev::read_data(void **buffer, int tmo_ms)
{
    if (simulator_ != nullptr) {
        ...
        if ((mu2e_mmap_ptrs_[0][0][0] != NULL) || ((retsts = init()) == 0))
        {
            if (buffers_held_ >= 2)
                read_release( 1 );
            has_recv_data = mu2e_chn_info_delta_(0, C2S, &mu2e_channel_info_);
            ...
            if ((has_recv_data > 0)
                ...
                && (has_recv_data = mu2e_chn_info_delta_(0, C2S, &mu2e_channel_info_)) > 0))
            {   // have data
...
int mu2edev::write_data(void *buffer, size_t bytes)
{
    ...
    else
    {
        ...
        int dir = S2C;
        int retsts = 0;
        unsigned delta = mu2e_chn_info_delta_(0, dir, &mu2e_channel_info_); // check cached info
        ...
        delta = mu2e_chn_info_delta_(0, dir, &mu2e_channel_info_);
        }
        if (delta > 0)
