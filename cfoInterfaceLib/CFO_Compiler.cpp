 ...
 *********************************************/
void CFOLib::CFO_Compiler::readLine()//Reads one line and stores into instruction, parameter, argument and identifier Buffers.
{
...
std::string CFOLib::CFO_Compiler::readInstruction() //reads on space separated word (with an exception for DATA REQ and DO LOOP)
{
	char insChar;
...
//*****************************
void CFOLib::CFO_Compiler::feedInstruction(std::string instruction, std::string argument, int64_t parameter, std::string identifier)
{
...
//Switch Conversions
CFOLib::CFO_Compiler::CFO_INSTR CFOLib::CFO_Compiler::parse_instruction(std::string instructionBuffer) //Turns strings into integers for switch statements.
{
	if (instructionBuffer == "START") { return CFO_INSTR::START; }
	else if (instructionBuffer == "DATA_REQUEST") { return CFO_INSTR::DATA_REQUEST; }
	else if (instructionBuffer == "INC") { return CFO_INSTR::INC; }
	else if (instructionBuffer == "SET") { return CFO_INSTR::SET; }
	else if (instructionBuffer == "AND") { return CFO_INSTR::AND; }
	else if (instructionBuffer == "OR") { return CFO_INSTR::OR; }
	else if (instructionBuffer == "LOOP") { return CFO_INSTR::LOOP; }
	else if (instructionBuffer == "DO_LOOP") { return CFO_INSTR::DO_LOOP; }
	else if (instructionBuffer == "REPEAT") { return CFO_INSTR::REPEAT; }
	else if (instructionBuffer == "WAIT") { return CFO_INSTR::WAIT; }
	else if (instructionBuffer == "END") { return CFO_INSTR::END; }
	return CFO_INSTR::INVALID;
}

//Macro Switch
CFOLib::CFO_Compiler::CFO_MACRO CFOLib::CFO_Compiler::parse_macro(std::string instructionBuffer) //Turns strings into integers for switch statements.
{
	if (instructionBuffer == "SLICE") { return CFO_MACRO::SLICE; }
...
void CFOLib::CFO_Compiler::errorCheck(CFO_INSTR instructionOpcode) //Checks if there are any misplaced arguments
{
	...
	case CFO_INSTR::START:
		if (parameterBuffer_ == 0)
		{
			std::cout << "ERROR: On Line " << lineNumber_
				<< ". START has a null or invalid argument (START 0 Event)" << std::endl;
			throw std::exception();
		}
		if (argumentBuffer_.empty() == false && argumentBuffer_ != "event_mode")
		{
			std::cout << "ERROR: On Line" << lineNumber_
				<< ". did you mean \"START event_mode=\"?" << std::endl;
			throw std::exception();
		}
		break;
	...
	case CFO_INSTR::DATA_REQUEST:
		...
		{
			std::cout << "ERROR: On Line " << lineNumber_
				<< ". DATA REQ has a null  or invalid argument (DATA REQ 0 Event)" << std::endl;
			throw std::exception();
		}
		else if (argumentBuffer_.empty() == false && argumentBuffer_ != "request_tag"
			&& argumentBuffer_ != "CURRENT")
		{
			std::cout << "ERROR: On Line" << lineNumber_
	...
	case CFO_INSTR::INC:
		...
		{
			std::cout << "WARNING: On Line " << lineNumber_
				<< ". INC has an extraneous identifier" << std::endl;
		}
		else if (argumentBuffer_.empty() == false && argumentBuffer_ != "event_by"
			&& argumentBuffer_ != "event_tag")
		{
			std::cout << "ERROR: On Line" << lineNumber_
				<< ". did you mean \"INC event_by=\" or \"INC event_tag\?" << std::endl;
			throw std::exception();
		}
		break;
	case CFO_INSTR::SET:
		if (parameterBuffer_ == 0)
		{
			std::cout << "\nERROR: On Line " << lineNumber_
				<< ". SET has a null  or invalid argument (SET 0 Event)" << std::endl;
			throw std::exception();
		}
		else if (argumentBuffer_.empty() == false && argumentBuffer_ != "event_tag")
		{
			std::cout << "ERROR: On Line" << lineNumber_
				<< ". did you mean \"SET event_tag=\"?" << std::endl;
			throw std::exception();
		}
		break;
	case CFO_INSTR::AND:
		...
		{
			std::cout << "\nERROR: On Line " << lineNumber_
				<< ". AND has a null  or invalid argument (AND 0 Event)" << std::endl;
			throw std::exception();
		}
		else if (argumentBuffer_.empty() == false && argumentBuffer_ != "event_tag")
		{
			std::cout << "ERROR: On Line" << lineNumber_
				<< ". did you mean \"AND event_tag=\"?" << std::endl;
			throw std::exception();
		}
		break;
	case CFO_INSTR::OR:
		...
		{
			std::cout << "\nERROR: On Line " << lineNumber_
				<< ". OR has a null  or invalid argument (OR 0 Event)" << std::endl;
			throw std::exception();
		}
		else if (argumentBuffer_.empty() == false && argumentBuffer_ != "event_tag")
		{
			std::cout << "ERROR: On Line" << lineNumber_
				<< ". did you mean \"OR event_tag=\"?" << std::endl;
			throw std::exception();
		}
		break;
	case CFO_INSTR::LOOP:
		...
		{
			std::cout << "ERROR: On Line " << lineNumber_
				<< ". LOOP has a null  or invalid argument (Looping 0 times)" << std::endl;
			throw std::exception();
		}
		else if (identifierBuffer_.empty() == 0 && identifierBuffer_ != "times")
		{
			std::cout << "ERROR: On Line " << std::to_string(lineNumber_)
				<< ". LOOP has an invalid identifier ("
		...
		{
			std::cout << "ERROR: On Line" << lineNumber_
				<< ". did you mean \"LOOP count=\"?" << std::endl;
			throw std::exception();
		}
		break;
	case CFO_INSTR::DO_LOOP:
		if (parameterBuffer_ != 0)
		{
			std::cout << "WARNING: On Line " << std::dec << lineNumber_
				<< ". DO LOOP has an extraneous argument(" << parameterBuffer_ << ")" << std::endl;
		}
		break;
	case CFO_INSTR::WAIT:
		if (identifierBuffer_.empty() == 0 && identifierBuffer_ != "cycles"
			&& identifierBuffer_ != "ns" && identifierBuffer_ != "sec"
			&& identifierBuffer_ != "ms")
		{
			std::cout << "WARNING: On Line " << std::dec << lineNumber_
				<< ". WAIT has an invalid identifier(" << identifierBuffer_ << ")" << std::endl;
			std::cout << "Options for third argument are [empty] ns ms sec" << std::endl;
		}
		else if (argumentBuffer_.empty() == false && argumentBuffer_ != "period" &&
			argumentBuffer_ != "NEXT")
		{
			std::cout << "ERROR: On Line" << lineNumber_
				<< ". did you mean \"WAIT period=\"?" << std::endl;
			throw std::exception();
		}
	...
	case CFO_INSTR::REPEAT:
		...
		{
			std::cout << "WARNING: On Line " << std::dec << lineNumber_
				<< ". REPEAT has extraneous arguments(" << parameterBuffer_
				<< "_" << identifierBuffer_ << ")" << std::endl;
		}
		break;
	case CFO_INSTR::END:
		if (parameterBuffer_ != 0 && identifierBuffer_.empty() == 1)
		{
			std::cout << "WARNING: On Line " << std::dec << lineNumber_
				<< ". END has extraneous arguments(" << parameterBuffer_
				<< "_" << identifierBuffer_ << ")" << std::endl;
		}
		break;
	default:
...
void CFOLib::CFO_Compiler::macroErrorCheck(CFO_MACRO macroInt)
{
	...
	case CFO_MACRO::SLICE:
		...
		else if ((atoi(macroArgument_[3].c_str()) + atoi(macroArgument_[1].c_str()) - 1 > 48) || (atoi(macroArgument_[3].c_str()) < 1))
		{
			std::cout << "ERROR: On Line (" << lineNumber_ << ")";
			throw std::runtime_error("4th parameter, integer must be between 1 and must add with bitposition to less than 48");
		}
...
int64_t CFOLib::CFO_Compiler::calcParameter(CFO_INSTR instructionOpcode)  //Calculates the parameter for the instruction
											  //(if needed)
{
	int64_t loopLine;
	switch (instructionOpcode)
