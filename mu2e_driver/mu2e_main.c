...
int checkDmaEngine(int dtc, unsigned chn, unsigned dir)
{
	...
	if (dir == C2S && (status & (DMA_ENG_INT_ALERR | DMA_ENG_INT_FETERR | DMA_ENG_INT_ABORTERR | DMA_ENG_INT_CHAINEND)) != 0)
	{
		...
		while ((status & DMA_ENG_USER_RESET) != 0 && lc > 0) { status = Dma_mReadChnReg(dtc, chn, dir, REG_DMA_ENG_CTRL_STATUS); --lc; }
		lc = 5;
		Dma_mWriteChnReg(dtc, chn, dir, REG_DMA_ENG_CTRL_STATUS, DMA_ENG_RESET);
		while ((status & DMA_ENG_RESET) != 0 && lc > 0) { status = Dma_mReadChnReg(dtc, chn, dir, REG_DMA_ENG_CTRL_STATUS); --lc; }
		sts = 1;
	}
	...
	{
		TRACE(20, "checkDmaEngine: DMA Engine Status: dtc=%d, chn=%d dir=%d r=%d, w=%d", dtc, chn, dir, ((status & DMA_ENG_RUNNING) != 0 ? 1 : 0), ((status & DMA_ENG_WAITING) != 0 ? 1 : 0));
	}
	return sts;
}
	...
	case M_IOC_GET_INFO:
		...
		else
		{
			...
			while (((mu2e_buffdesc_S2C_t*)idx2descVirtAdr(hwIdx, dtc, chn, dir))->Complete
				   && hwIdx != mu2e_channel_info_[dtc][chn][dir].swIdx
				   && jj--)
			{
		...
		for (chn = 0; chn < 2; ++chn)
			for (dir = 0; dir < 2; ++dir)
			{
				...
				TRACE(10, "chn=%d dir=%x (0x%08x) hw_next_idx=%u sw_next_idx=%u hw_cmplt_idx=%u"
					  ...
					  , hw_next
					  , descDmaAdr2idx(hw_next, dtc, chn, dir, 0)
					  , descDmaAdr2idx(sw_next, dtc, chn, dir, 0)
					  , descDmaAdr2idx(hw_cmplt, dtc, chn, dir, 0));
				if (dir == 0) // C25
				{
