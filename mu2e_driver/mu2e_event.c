...
static void poll_packets(unsigned long dc)
{
	...
	{   // Read the HW register and convert (Dma) addr in reg to idx.
	  u32 newCmpltIdx = descDmaAdr2idx(Dma_mReadChnReg(dtc, chn, dir, REG_HW_CMPLT_BD), dtc, chn, dir, mu2e_channel_info_[dtc][chn][dir].hwIdx);

		u32 do_once = 0;

		if (newCmpltIdx >= MU2E_NUM_RECV_BUFFS)
		{
			TRACE(0, "poll_packets: newCmpltIdx (0x%x) is above maximum sane value!!! (%x) Current idx=0x%x", newCmpltIdx, MU2E_NUM_RECV_BUFFS, mu2e_channel_info_[dtc][chn][dir].hwIdx);
			TRACE_CNTL("modeM,0");
			error = 1;
			//continue;
			break;
		...
		TRACE(21, "poll_packets: MU2E_NUM_RECV_BUFFS=%i newCmpltIdx=0x%x Current_hwIdx=0x%x"
		      , MU2E_NUM_RECV_BUFFS, newCmpltIdx, mu2e_channel_info_[dtc][chn][dir].hwIdx);
...
{
	del_timer_sync(&packets_timer[dtc]);
}
